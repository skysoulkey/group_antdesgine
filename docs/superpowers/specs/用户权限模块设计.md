...
| 操作者 | 创建入口 | 可创建的角色 |
|--------|---------|-------------|
| `group_owner` | 公司清单 → 创建公司 | `company_owner`（公司主，随公司一起创建） |
| `group_owner` | 用户管理 → 创建用户 | `group_finance`, `group_ops`, `group_audit`（仅集团侧） |
| `company_owner` | 用户管理 → 创建用户 | `company_promo`, `company_finance`, `company_ops`, `company_audit`（仅公司侧） |
...
- `group_owner` 在用户管理中只创建集团侧管理员，公司主通过创建公司时一起创建
