...
- 操作列按钮：`订单详情`、`返佣详情`，均为 `<Button type="link" size="small" style={{ padding: 0 }}>`，间距 `Space size={4}`
- 订单状态列：黑色纯文本（已开奖 / 未开奖），不使用 Tag
- 玩法、底注、红包数量等纯文本列：黑色文本，不滥用 Tag
- 金额列（赔付金额 / 抽水金额 / 返佣金额 / 本局盈亏 / 到账 / 抽水）：黑色 `#141414`，不分正负色
- 完成时间：未开奖时显示 `—`
- 订单详情 Modal：宽度 1000px，含「概要区 + 领取记录表」
- 返佣详情 Modal：宽度 800px，单表 7 列
