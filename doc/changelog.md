...
### 调整：公司清账单表重做（字段精简 + 扣款状态 3 态化 + 手动扣款）

**变更**
- 字段精简到 9 列：订单时间 / 订单号 / 账单周期 / 游戏 / 应用费用 USDT / 应用费用 PEA / 扣款状态 / 备注 / 操作
- 删除：账单类型、游戏收益 USDT/PEA、扣款时间（合并到订单时间）
- 扣款状态改为 3 态：**待扣款 / 扣款成功 / 扣款失败**（原 2 态待扣/已扣废弃）
- 操作列：[预览账单 PDF] 始终在 + [手动扣款] 仅扣款失败显示（占位 message，待后端联调）
- 默认排序：账单周期降序（新月在上）
- 备注列：后端写入的自由文本，前端只读
- 应用费金额始终展示应收口径，状态由「扣款状态」列承担
- mock 改造：本月 2026-04 扣款失败演示重试场景；上月扣款成功；早期月份待扣款演示跨月欠费

**影响范围**
- `src/pages/finance/settlement/index.tsx`：列定义、状态枚举、mock、筛选
- `doc/page_map.md`：字段表更新
- `src/pages/finance/settlement/preview.tsx`：保留不动

集团端 `/enterprise/app-fee` 保持不变。

## 2026-04-30

### 调整：公司钱包页流水表收敛为「划转流水」纯净视图

**变更**
- `/finance/all-wallet` 流水表只保留划转流水（OrderType: transfer），删除追加投资 / 释放股份 / 转单扣款 3 种类型的 mock 数据、筛选 Radio、详情弹窗分支
- 流水表列：订单时间 · 流水号 · **划转方向（转出 → 转入）** · 金额 · 币种 · 操作人 · 状态 · 操作
- 流水表标题改为「划转流水」（原"流水记录"）
- 筛选区简化为「钱包 + 订单时间」两项；钱包筛选语义改为"从或到任一端命中"
- 划转弹窗（一步填表 → 二步 MFA 确认）保持现状不动
- 详情弹窗收敛为单一形态（划转专用字段）

**改动文件**
- `src/pages/finance/all-wallet/index.tsx`（整体重写：删除 OrderType 枚举 / sourceCompany / sourceOwner / commissionOrder / MOCK_ENTERPRISES 等无关结构）
- `doc/page_map.md`（更新该页字段清单）

**风险**
- mock 流水重置：演示前需注意流水从空白状态填充，以划转 mock 数据展示
