...
### 审批中心（`/finance/approvals`）
- **审批列表 Tab**：筛选（事件类型 Radio + 审批状态 Radio + 企业 Select + 日期范围）、完整审批单字段表格、通过/拒绝操作（二次确认弹窗）、查看详情弹窗
- **审批规则 Tab**：公司级自动审批规则配置，支持按金额/企业/金额+企业组合条件，新增/编辑/启用停用/删除
